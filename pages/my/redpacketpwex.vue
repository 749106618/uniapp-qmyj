<template>
	<view>
		...
		<view class="i wwi-icon wwi-icon-hongbao"></view>
			<view class="dl rule">
				<view class="dd">请输入已知平台红包卡密号码</view>
				<view class="dd">确认生效后可在购物车使用抵扣订单金额</view>
			</view>
	</view><view class="box-ling">1</view>
		</view>
		<view class="wwiapp-log-icon">
			<navigator class="icon-li" url="/pages/my/redpacketlist" open-type="redirect"><view class="wwi-icon wwi-icon-hongbao color-cs"></view><text>红包列表</text></navigator>
			<view class="icon-li color-hui" ><view class="wwi-icon wwi-icon-zijinxunhuan"></view><text>领取红包</text></view>
		</view>
		<view class="content">
		<view class="input-group">
			<view class="input-row border">
				<text class="title">红包卡密</text>
				<input type="text" v-model="pwdCode" pattern="[0-9]*" placeholder="请输入平台红包卡密号">
			...
			<view class="input-row border">
				...
				<view class="btn-row">
					<button type="primary" class="packet" @tap="nextGo" :disabled="pwdCode.length>0 &&code.length > 0 ? false:true">提交</button>
				</view>
		</view>
	</view>
</template>
