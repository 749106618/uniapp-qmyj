	...
	<view>
		...
		<view class="wwiapp-log-list">
			...
			<view class="li" v-for="(item,index) in log_list" :key="index">
				<view class="detail">{{item.lg_desc}}
				</view>
                
                <text v-if="item.lg_av_amount>0" class="money add">+{{item.lg_av_amount}}</text>
				<text v-else class="money reduce">{{item.lg_av_amount}}</text>
                
                <text class="date">{{item.lg_add_time_text}}</text>
            </view>
