	...
	<view>
		...
		<view class="rpt-hd">
			<view class="title flex flexAic flexrow">
				￥<view class="price">{{info.voucher_price}}</view> <view class="state">{{info.voucher_state_text}}</view>
			</view>
			<view class="dl flex flexrow flexAic">
				<view class="dt">红包编号</view>
				<view class="dd">{{info.voucher_code}}</view>
			</view>
			<view class="dl flex flexrow flexAic">
				<view class="dt">有&nbsp;效&nbsp;期</view>
				<view class="dd">{{info.voucher_start_date_text}}-{{info.voucher_end_date_text}}</view>
			</view>
			<view class="dl flex flexrow flexAic">
				<view class="dt">使用规则</view>
				<view class="dd flex flexrow"><view class="span" v-if="info.voucher_limit">满{{info.voucher_limit}}可用</view>
							<view class="span" v-else>不限制金额可用</view></view>
			</view>
			<view class="dl flex flexrow flexAic">
				<view class="dt">特殊限制</view>
				<view class="dd">{{info.voucher_usetype_text}}</view>
			</view>
			<view class="dl flex flexrow flexAic" v-if="info.voucher_usetype=='2'">
				<view class="dt">限制分类</view>
				<view class="dd flex flexrow">
					<navigator class="em" :url="'/pages/web/glist?gcid='+gc.gc_id" v-for="(gc,index) in info.voucher_useinfo" :key="index">{{gc.gc_name}}</navigator>
				</view>
			</view>
		</view>
		<view class="rpt-goods-list" v-if="info.voucher_usetype=='3'">
			<view class="li flex flexrow" v-for="(item,index2) in info.voucher_useinfo" :key="index2">
...
<script>
	...
	export default {
		...
		methods:{
			loadData:function(){
				this.wwiopt({
					url: '/index.php?app=member_voucher&wwi=voucher_info',
					method: 'POST',
					data: {key:this.key,voucher_id:this.voucherId},
					success: res => {
						this.info = res.data.datas.voucher_info;
					}
				});
			},
			getGoodsmask(e){
