<template>
	<view>
        <view class="h-tips">返利佣金=支付金额*佣金比例/当前系数*当前系数</view>

		<view class="bill-list">
		<view class="h-tips" v-if="buyerID">正在查看下级会员ID:{{buyerID}}所有结算 <view style="color: #00BCD4;" @tap="stateOrder('')">》》点击查看全部下级结算</view></view>
				<view v-if="billList.length<1" class="wwi-nodata" style="position:relative;">
					<view class="no-main">
			...
			<view class="li" v-for="(item,index) in billList" :key="index">
				<view class="bill-sn flex flexJcs">
					...
					<view class="state">{{item.bill_state_txt}}</view></view>
				<view class="goods-info flex flexrow">
					<image :src="item.goods_image_url" ></image>
					<view class="goods-name">{{item.goods_name}}
					<view class="em flex flexrow">{{item.invite_id}}级会员 <view class="p" @tap="stateOrder(item.buyer_id)">{{item.buyer_name}}</view></view>
					</view>
				</view>
				<view class="flex flexrow">
					<view class="price">
						<view class="tit">支付金额</view>
						<view class="bti">{{item.pay_goods_amount}}</view>
					</view>
					<view class="price">
						<view class="tit">退款金额</view>
						<view class="bti">{{item.refund_amount}}</view>
					</view>
					<view class="price">
						<view class="tit">佣金比例</view>
						<view class="bti">{{item.invite_commis_rate}}%</view>
					</view>
					<view class="price">
						<view class="tit">当前系数</view>
						<view class="bti">{{item.invite_rate}}</view>
					</view>
					<view class="price">
						<view class="tit">总系数</view>
						<view class="bti">{{item.invite_all_rate}}</view>
					</view>
				</view>
				<view class="handle-bill flex flexJcs">
					<view class="batch-price flex flexrow">分销佣金:<view class="em">￥{{item.invite_pay_amount}}</view></view>
					<view class="time" v-if="item.invite_pay_time">已结时间:{{item.invite_pay_time}}</view>
				</view>
			</view>
