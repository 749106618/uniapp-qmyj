		...
		<view class="goods-list">
				<view v-if="!goodsList.length" class="wwi-nodata">
					<view class="no-main">
						...
						<view class="dl">
							<view class="dt">没有相关限时商品</view>
							<view class="dd">选择其他分类试试哦~</view>
						</view>
					</view>
				</view>
			<navigator :url="'/pages/web/item?goodsid='+vv.goods_id" class="item" v-for="(vv,v) in goodsList" :key="v">
				<view class="pic"><image :src="vv.image_url_240" lazy-load></image></view>
				<view class="goods-info">
					<view class="goods-name">{{vv.goods_name}}</view>
					<view class="goods-pinfo">{{vv.xianshi_title}}</view>
					<view class="price flex flexrow"><view class="em">促销价</view><view class="span flex flexrow">&yen;<view class="i">{{vv.xianshi_price}}</view></view></view>
					<view class="del flex flexrow"><view class="wwi-icon wwi-icon-jiantou_qiehuanxiangxia"></view>直降{{vv.down_price}}</view>
				...
				this.wwiopt({
					...
					success: res => {
						if(res.data.code==200){
							this.curpage +=1;
							for(let item in res.data.datas.goods_list){
								this.goodsList.push(res.data.datas.goods_list[item]);
							}
...
.goods-list .item .goods-info .goods-pinfo{display: inline-block;padding: 0 20upx;height: 30upx;line-height: 30upx;color: #fff;font-size: 24upx;background-color: #ffc107;border-radius: 16upx;vertical-align: top;}
.goods-list .item .goods-info .price{bottom: 60upx;line-height: 30upx; position: absolute; left: 0; color: #FF5722;}
...
.goods-list .item .goods-info .del{ position: absolute;bottom: 20upx;left: 0;color: #1BBC9D;font-size: 24upx;max-width: 200upx; align-items: baseline;}
.goods-list .item .goods-info .btn {position: absolute;bottom: 30upx;right: 30upx;width: 160upx;height: 60upx;line-height: 60upx;color: #fff;font-size: 28upx;text-align: center;border-radius: 50upx;background: -webkit-gradient(linear,left top,right top,from(#FF5722),to(#FFC107));background: -webkit-linear-gradient(left,#FF5722,#FFC107);background: linear-gradient(90deg,#FF5722,#FFC107);box-shadow: 0 12upx 20upx rgba(233,59,61,.2);}
</style>
