<template>
	<view class="newBody">
		...
		<view class="wwiapp-cart-container flex flexcol" v-for="(item,index) in cartList.cart_list" :key="index" v-if="cartList.cart_list.length > 0">
			...
			<view class="wwiapp-cart-item flex flexcol">
				<view class="cart-litemw-cnt" :class="{'jjg':kk.jjgRank> 0}" v-for="(kk,vv) in item.goods" :key="vv">
					...
					<view  class="goods-pic"><image :src="kk.goods_image_url"></image></view>
					<view class="goods-info flex flexcol">
						<view class="goods_name flex flexrow"><view class="goods-name">{{kk.goods_name}}</view>
		...
		<view class="bottom flex flexrow" style="bottom: 100upx;"  v-if="cartList.cart_list.length > 0 &&key">
		...
		<view class="foot-bottom flex flexAic flexrow">
			...
			<view class="li cur flex flexAic flexJcc flexcol"><view class="wwi-icon wwi-icon-baoguo_yidong"></view>购物车</view>
...
.wwiapp-cart-item .cart-litemw-cnt .goods-pic{width: 140upx; height: 140upx; border-radius:14upx; padding: 10upx;    border: solid 2upx #EEE;}
