<template>
	<view>
		...
		<view class="integral-02">
				<view v-if="!voucherList.length" class="wwi-nodata">
					<view class="no-main">
			...
			<view class="integral-list" v-if="voucherList.length">
	...
	export default {
		...
		methods:{
			Inlimine(action = 'refresh') {
				...
				this.wwiopt({
					url: '/index.php?app=points&wwi=exchange_voucher',
					data: {
						page: this.page,
					...
					success: res => {
						if (res.data.code == '200') {
							this.curpage += 1;
							if (action == 'onbot') {
								res.data.datas.voucherlist.forEach(item => {
									item.bfb = parseFloat((item.voucher_t_giveout / item.voucher_t_total * 10000) / 100.00).toFixed(2);
						...
						res.data.datas.voucherlist.forEach(item => {
							...
							this.$set(item, 'bfb', num);
								...
								this.voucherList = res.data.datas.voucherlist;
			...
			if(parseInt(e.voucher_t_giveout) < parseInt(e.voucher_t_total)){
				if(parseInt(e.voucher_t_points) > parseInt(this.Minfo.member_points)){
				uni.showToast({	title: '您的积分不足，暂时不能兑换该代金券',icon:"none"});
				return false;
				}
				if(parseInt(e.voucher_t_mgradelimit) > parseInt(this.Minfo.level)){
				uni.showToast({	title: '您的会员级别不够，暂时不能兑换该代金券',icon:"none"});
				return false;
				}
				var b_msg = '';
				b_msg +='店铺('+e.voucher_t_storename+')'+e.voucher_t_price+'元店铺代金券，消费满'+e.voucher_t_limit+'元减'+e.voucher_t_price+'元，有效期至'+e.end_date+',';
				var limit_count = '不限量';
