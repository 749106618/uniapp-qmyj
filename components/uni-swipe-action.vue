	...
	<view class="uni-swipe-action">
		<view class="uni-swipe-action__container" :class="!isMoving ? 'animtion' : ''" @touchstart="touchStart" @touchmove="touchMove" @touchend="touchEnd" @touchcancel="touchEnd" @click="bindClickCont" :style="{'transform':transformX,'-webkit-transform':transformX}">
			<view class="uni-swipe-action__content">
				<view class="li">
					<navigator :url="'/pages/my/chatinfo?tid='" class="avater"><image :src="dataload.avatar" mode=""></image>
					<view class="sup" v-if="dataload.r_state == 2"></view></navigator>
					<navigator :url="'/pages/my/chatinfo?tid='+dataload.u_id" class="info">
						<view class="dt">{{dataload.u_name}}<view class="time">{{dataload.time}}</view></view>
						<view class="dd">{{dataload.t_msg}}</view>
					</navigator>
				</view>
			</view>
